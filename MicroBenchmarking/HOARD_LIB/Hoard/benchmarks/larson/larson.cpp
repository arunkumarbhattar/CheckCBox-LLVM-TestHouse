...
extern "C" void * hoard_malloc(size_t) __attribute__((alloc_size(1))) ;
extern "C" void hoard_free(void *);
extern "C" int isPointerinHeap(void *p) ;
...
#else
  printf("C version (hoard_malloc and hoard_free)\n") ;
#endif
  printf("runtime (sec): ") ;
  scanf ("%ld", &sleep_cnt);
...
#else
    blkp[cblks] = (char *) hoard_malloc(blk_size) ;
  bool isValid = isPointerinHeap(blkp[cblks]);
  if(isValid)
  {
      printf("Pointer is in heap\n");
  }
  else
  {
      printf("Pointer is not in heap\n");
  }
#endif
...
#else
      blkp[victim] = (char *) hoard_malloc(blk_size) ;
        bool isValid = isPointerinHeap(blkp[victim]);
        if(isValid)
        {
            printf("Pointer is in heap\n");
        }
...
#else
    pdea->array[victim] = (char *) hoard_malloc(blk_size) ;
#endif
...
#else
    blkp[cblks] = (char *) hoard_malloc(blk_size) ;
    //lets try and allocate from malloc 
    char* malloc_ptr = (char*)malloc(blk_size);
    char* stack_ptr[blk_size];
