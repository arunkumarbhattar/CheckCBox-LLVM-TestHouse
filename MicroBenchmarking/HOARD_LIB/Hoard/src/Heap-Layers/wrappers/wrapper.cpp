...
extern "C" void MYCDECL hoard_free(void *);
extern "C" void * MYCDECL hoard_malloc(size_t) __attribute__((alloc_size(1))) __attribute((malloc));
extern "C" void * MYCDECL CUSTOM_CALLOC(size_t nelem, size_t elsize) __attribute__((alloc_size(1,2)))  __attribute((malloc));

extern "C" FLATTEN void MYCDECL hoard_free (void * ptr)
{
  xxfree (ptr);
}

extern "C" FLATTEN void * MYCDECL hoard_malloc(size_t sz)
{
  void * ptr = xxmalloc(sz);
  return ptr;
}

extern "C" FLATTEN bool MYCDECL isPointerinHeap(void* ptr)
{
  return xxisPointerinHeap(ptr);
}
